...
\\[2pt]
\scriptsize {\lolit Slides:} \href{http://bit.ly/StPaul-0}{\tt \scriptsize
  \color{foreground} bit.ly/StPaul-0}
}
...
\frame{
  ...
  \note{These are slides for a one-hour workshop on Reproducible
    Research, part of the Data Science and Plant
    Breeding Simulation Workshop at the University of Minnesota on 23
    March 2017.

    Source: {\tt https://github.com/kbroman/RR\_Workshop} \\
    Slides: {\tt http://bit.ly/StPaul-0-nonotes} \\
    With notes: {\tt http://bit.ly/StPaul-0}
}
} }


\begin{frame}[c]{Preparations}

\centering
{\large Install R, RStudio, and some R packages.}

\vspace{1cm}

{\large \href{http://bit.ly/StPaul-prep}{\tt bit.ly/StPaul-prep}}



\note{
    For Activity 3, we will be creating an R Markdown document, which
    mixes text and code to make a fully reproducible report.

    You will first need to install, R, RStudio, and some R
    packages. It's best to have the latest versions, so if you
    installed them in the distant past but haven't updated them
    recently, please re-install them.
}
\end{frame}
...
\note{Computational work is
  ...
  someone repeat the experiment and get equivalent results?
...
\note{The above website contains my thoughts on how to move toward full
  reproducibility.
...
\begin{frame}[c]{1. Arrange data to ease analysis}

\begin{center}
\large
Write programs for people \\[18pt]
Organize data for computers
\end{center}

\note{
  One may be tempted to think that, when you're writing a computer
  program, you're trying to communicate with the computer. But
  while it's important that the program executes correctly, it is just
  as important that it is \emph{readable}. Then others (which might be
  you, three months from now) will be better able understand the code,
  for example to fix problems, or to build upon it.

  On the other hand, when laying out data within a spreadsheet, you
  should think principally about having it be easily read by the
  computer; this will make analysis easier.
}
\end{frame}


\begin{frame}[c]{Activity 1}

\begin{center}
\large
Arrange data to ease analysis \\[18pt]

\href{http://bit.ly/StPaul-1}{\tt bit.ly/StPaul-1}

\end{center}

\note{
   First activity: study the data in a spreadsheet, and discuss how
   the arrangement could be improved.
}
\end{frame}



\begin{frame}{Activity 1: original file}

\vspace{7mm}

\figh{Figs/activity1_ugly.pdf}{0.6}

\vspace{10mm}

\hfill
{\footnotesize \lolit
\href{http://bit.ly/St_Paul-1}{\tt bit.ly/StPaul-1}
}

\note{
   Here's an image of the original file.
   What could be improved?

   After studying this for a bit, you can likely figure out what the
   data mean, but a computer is going to have a harder time.
   If you read this into R and want to make some plots, you're going
   to have to spend a good amount of time reorganizing the data.
}
\end{frame}

\begin{frame}{Activity 1: tidy file}

\vspace{7mm}

\figh{Figs/activity1_tidy.pdf}{0.6}

\vspace{10mm}

\hfill
{\footnotesize \lolit
\href{http://bit.ly/St_Paul-1-tidy}{\tt bit.ly/StPaul-1-tidy}
}
...
\note{
  ...
  The key thing: the data are now arranged as a rectangle, with the
  rows being observations and columns being variables. If you read
  this version into R, you'll be able to start making plots right
  away.
}
\end{frame}
...
\hfill
{\footnotesize \lolit
\href{http://kbroman.org/dataorg}{\tt kbroman.org/data.org}
}
...
\note{
  A summary of considerations in the organization of data in
  spreadsheets. See \href{http://kbroman.org/data.org}{\tt kbroman.org/data.org}
}
\end{frame}
...
\note{Step 2 is to make your project
...
\note{
   Second activity: given a list of files for a project, how would you
   organize them? Would you rename some of the files?
}
...
\note{
   Third activity: we'll open R within RStudio and create an RMarkdown
   report. The link above is an example.
}
...
\onslide<2|handout 0>{\vspace{24mm}}
